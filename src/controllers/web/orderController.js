...
exports.downloadsecondforBFSBC = function (req, res, next) {
  var pageName = "Order List";
  var pageTitle = req.app.locals.siteName + " - " + pageName + " List";
  let isAdminLoggedIn = (typeof req.session.admin != "undefined") ? req.session.admin.userId : "";
  const orderId = req.params.id;
...
exports.returninvoicesbr = function (req, res, next) {
  var pageName = "Order List";
  var pageTitle = req.app.locals.siteName + " - " + pageName + " List";
  let isAdminLoggedIn = (typeof req.session.admin != "undefined") ? req.session.admin.userId : "";
  ...
  Order.aggregate([
    ...
    if (error) {
      ...
      try {
        ...
        const loginHtmlPath = 'views/webpages/new-Return-Invoice-SBR(1).html';
        const htmlTemplate = fs.readFileSync(loginHtmlPath, 'utf-8');
        const orderDate = new Date(orderList[0].added_dtime);
        const formattedDate = `${orderDate.getDate()}-${orderDate.getMonth() + 1}-${orderDate.getFullYear()}`;
        const product = orderList[0].product[0];
...
exports.shippingkitpdf = function (req, res, next) {
  var pageName = "Order List";
  var pageTitle = req.app.locals.siteName + " - " + pageName + " List";
  let isAdminLoggedIn = (typeof req.session.admin != "undefined") ? req.session.admin.userId : "";
  ...
  Order.aggregate([
    ...
    if (error) {
      ...
      try {
        const loginHtmlPath = 'views/webpages/new-BFS-TO-SELLER-FOR-SHIPPING KIT.html';
        const htmlTemplate = fs.readFileSync(loginHtmlPath, 'utf-8');
        const orderDate = new Date(orderList[0].added_dtime);
        const formattedDate = `${orderDate.getDate()}-${orderDate.getMonth() + 1}-${orderDate.getFullYear()}`;
